...
import {enableAutoClosing} from "../autoClosing";
import {disableAutoClosing} from "../autoClosing";
 ...
 * - push the requested popup to history
 */
const open = <ID extends AnyID>({id, reference, payload, zIndex, toggler}: Request<ID>) => {
 ...
 *   - close all open popups
 * - else 
 *   - if there is an open popup with an id of `id`
 *     - remove it along with the ones opened after it from `HISTORY`
 *     - close it along with the ones opened after it
 *     - if `HISTORY` is not empty
 *       - open or re-open the popup represented by the last entry in `HISTORY`
 */
...
const join = <ID extends AnyID>({id, zIndex, toggler, payload, reference}: Request<ID>) => {
  push(Object.freeze({
    id: id, 
    zIndex: zIndex, 
    toggler: toggler, 
...
/**
 * - if the requested popup is closed
 *   - `open()` it
...
/**
 * - closes all open popups without clearing `HISTORY` (so you can `backward()` to them later)
 * - pushes the requested popup to `HISTORY`
 * - opens the requested popup
 */
const forward = <ID extends AnyID>({id, reference, payload, zIndex, toggler}: Request<ID>) => {
  dispatch(CLOSE, not(id));
...
export {open, close, join, toggle, forward, backward};
export default Object.freeze({open, close, join, toggle, forward, backward});
...
type BaseAction<Type extends string = string> = Readonly<{
  type: Type; 
  meta?: unknown;
  error?: unknown;
  payload?: unknown; 
...
>(
  ...
  const reducer = (state: State = getDefaultState(), action: Actions[keyof Actions]) => {
    const draft = createDraft(state);

    for (let [type, caseReducer] of map) {
      if (type === action.type) {

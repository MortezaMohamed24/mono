...
import MOVE_ALL_OWN_CARDS from "./action.js";
import {PopulateListByBody} from "src/models/list/api/middlewares";


MOVE_ALL_OWN_CARDS.push(PopulateListByBody<Config, "listB", "idListB">({
  listKey: "listB",
  idListKey: "idListB",
}));
...
MOVE_ALL_OWN_CARDS.push(async ({get, set}) => {
  const {listA, listB} = get();

  const cards = await listA.moveAllOwnCards(listB);

  set({cards});
});


MOVE_ALL_OWN_CARDS.push(async ({get}) => {
